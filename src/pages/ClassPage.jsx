...
const ClassPage = () => {
  ...
  const fetchAllClasses = async () => {
    try {
      ...
      const requests = mappedClasses.filter(
        (cls) => cls.status === "Chưa duyệt"
      );
  ...
  const approveClass = async (id) => {
    try {
      ...
      console.log(
        `Class with ID ${id} approved successfully:`,
      ...
      const requests = updatedClasses.filter(
        (cls) => cls.status === "Chưa duyệt"
      );
      const approved = updatedClasses.filter(
        (cls) => cls.status === "Đã duyệt"
      );
      setRequestClasses(requests);
      setApprovedClasses(approved);
    } catch (error) {
  ...
  return (
    <div className="flex-1 overflow-auto relative z-10">
      ...
      <main className="max-w-7xl mx-auto py-6 px-4 lg:px-8">
        ...
        />
        <ClassTable classes={approvedClasses} deleteClass={deleteClass} />
      </main>
    </div>
  );
};
